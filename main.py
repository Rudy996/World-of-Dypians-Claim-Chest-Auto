...
for index, line in enumerate(info_lines):
    ...
    for proxy in proxy_lines:
        ...
        try:
            ...
            if test_response.status_code == 200:
                ...
                proxy_valid = True
                break  # Exit the proxy loop on successful test
            else:
                print(f"Proxy {proxy} failed with status code {test_response.status_code}.")
        except:
            print(f"Proxy {proxy} is not working.")

    if not proxy_valid:
        print(f"No valid proxy found for {email}. Sleep 900000000 sec")
    ...
    for chestIndex in range(10):
        ...
        # Sending the POST request using the corresponding proxy
